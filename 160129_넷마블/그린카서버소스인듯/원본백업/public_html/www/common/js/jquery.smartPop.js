    ...
    $.smartPop = {
      ...
      //      $('html').css({ display: 'block', overflow: 'hidden', overflowY: 'hidden' });
        ...
        resize : function() {
			...
			this.log('conOptUse : ' + this.opts.conOptUse);
            this.log('');

            // 기본 설정
 //           $('#smartPop_container').css({ border: '#fff ' + this.opts.border + 'px ' + this.opts.borderColor });
...
//            $('#smartPop_content').css({ padding: this.opts.padding + 'px' });
            ...
            this.resizeHeight(this.opts.height,false);	/* 이 부분에서 깜박거림 */
        },
        resizeHeight : function(h,show) {
...
//			alert(h);
            ...
            if(this.opts.url == '') {
				...
				if (show == true){
					...
					$('#smartPop_frame').css({ width: (this.opts.width - this.opts.padding * 2) + 'px', height: (h - this.opts.padding * 2) + 'px' }).show();
				} else {
					$('#smartPop_frame').css({ width: 0 + 'px', height: 0 + 'px' }).show();
				}
            }
        ...
        install : function() {
            if(this.isInstall == false) {
				...
				if(this.opts.conOptUse == "Y"){
                var smartPop_container      = $('<div />').attr('id', 'smartPop_container').css({'background':'none', '-webkit-box-shadow':'0 0 0px'});
				} else {
                ...
                var smartPop_frame          = $('<iframe />').attr({ id: 'smartPop_frame', frameBorder: 0, scrolling: 'no', allowTransparency: false });
        ...
        close : function() {
            ...
            $('html').css({ marginRight: 0, display: ''});
    ...
    $.smartPop.defaults = {
        ...
        background  : '#000',
        closeMargin : 3,
        closeImg    : {width:13, height:13, src:'./himg/common/btn_pop_close1.png'},
		conOptUse	: 'N',
        opacity     : .7,
        width       : 720,
        height      : 500,
