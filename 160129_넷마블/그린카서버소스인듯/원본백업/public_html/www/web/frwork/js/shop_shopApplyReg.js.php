...
<!--
	...
	function goShopApplyRegCheck() {
		...
		if(strSiteJsLng == 'KR') {
			...
			if(!C_chkInput("com_addr2",true,"<?=$LNG_TRANS_CHAR['SW00015']?>",true)) return; //주소
			...
			//if(!C_chkInput("com_num1_1",true,"<?=$LNG_TRANS_CHAR['SW00012']?>",true)) return; //사업자번호
			//if(!C_chkInput("com_zip1",true,"<?=$LNG_TRANS_CHAR['SW00014']?>",true)) return; //우편번호
			if(!C_chkInput("com_state",true,"주",true)) return; //주
			if(!C_chkInput("com_city",true,"<?=$LNG_TRANS_CHAR['CW00130']?>",true)) return; //도시
			if(!C_chkInput("com_addr",true,"<?=$LNG_TRANS_CHAR['SW00015']?>",true)) return; //주소
		}
		...
		var strHtml = "";
			...
			strHtml += "		<h1><?= $LNG_TRANS_CHAR['OS00093']; //추가정보를 등록해 주세요. ?></h1>";
			...
			strHtml += "		<p>";
			strHtml += "			<?= $LNG_TRANS_CHAR['OS00094']; ?> ";
			//strHtml += "			세부정보 및 회사소개 정보를 추가로 등록하시면<br>";
			//strHtml += "			<strong>특별한 입점혜택</strong>을 드립니다.";
			strHtml += "		</p>";
			strHtml += "		<p class=\"chkTxt\"><?= $LNG_TRANS_CHAR['OS00095']; //등록하시겠습니까? ?></p>";
			strHtml += "	</div>";
			strHtml += "	<div class=\"popBtnWrap\">	";
			strHtml += "		<a href=\"javascript:goShopApplyDetailView();\" class=\"btn_ok\"><?= $LNG_TRANS_CHAR['OS00096']; //예 ?></a>";
			strHtml += "		<a href=\"javascript:goShopApplyDetailNo();\" class=\"btn_no\"><?= $LNG_TRANS_CHAR['OS00097']; //아니오 ?></a>";
			strHtml += "	</div>";
			strHtml += "</div>";
			strHtml += "</div>";
