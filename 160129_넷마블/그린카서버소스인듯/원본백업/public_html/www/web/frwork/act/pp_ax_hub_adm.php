...
<?
	...
	if (!$vcnt_yn) $vcnt_yn        = $_POST[ "vcnt_yn"        ]; // 가상계좌 에스크로 사용 유무
 ...
 //       $c_PayPlus->mf_set_modx_data( "mod_desc", $_POST[ "mod_desc" ] );			// 변경 사유
        ...
        else if ($mod_type == "STE2" || $mod_type == "STE4") // 상태변경 타입이 [즉시취소] 또는 [취소]인 계좌이체, 가상계좌의 경우
        {
			...
			if ($vcnt_yn == "Y")
            {
				$c_PayPlus->mf_set_modx_data( "refund_account",   $strReturnAcc );		// 환불수취계좌번호
                $c_PayPlus->mf_set_modx_data( "refund_nm",        $strReturnName);     // 환불수취계좌주명
	...
	$strLogText = "결제번호:".$orderMgr->getO_NO()."/DB오류:".$bSuccText."/res_cd=".$res_cd."/req_cd=".$req_cd."/req_tx=".$req_tx."/mod_type=".$mod_type."/res_cd=".$res_cd."/".ICONV("EUC-KR","UTF-8",$res_msg);
