...
ini_set('error_log', WEB_LOG_PATH.'/'.date("Y-m-d").'.ipn_errors.log');


// instantiate the IpnListener class
include(MALL_WEB_PATH.'frwork/pay/paypal/ipnlistener.php');
...
if ($verified) {
    ...
    */
	...
	$strPaymentStatus	= $listener->post_data["payment_status"];
	$strAuthId			= $listener->post_data["auth_id"];
	$strAuthStatus		= $listener->post_data["auth_status"];
	$intAuthAmt			= $listener->post_data["auth_amount"];
	$strTxnType			= $listener->post_data["txn_type"];
	$strReceiverId		= $listener->post_data["receiver_id"];
	
	$orderMgr->setO_APPR_NO($strAuthId);
	$intO_NO = $orderMgr->getOrderNo($db);
	$orderMgr->setO_NO($intO_NO);
	$orderRow = $orderMgr->getOrderView($db);
		
	if ($strPaymentStatus == "Completed" && $strAuthStatus == "Completed"){
		
		/* 배송중으로 상태변경 */		
		//if ($orderRow[O_TOT_SPRICE] == $intAuthAmt){
			$orderMgr->setO_STATUS("B");
			$orderMgr->getOrderStatusUpdate($db);
			$strSuccess = "OK";
		//}
		
		/* Receiver Id Update */
		$orderMgr->setO_PAYPAL_RECEIVER_ID($strReceiverId);
		$orderMgr->getOrderPayPalReceiverIdUpdate($db);

	} else if ($strPaymentStatus == "Voided" && $strAuthStatus == "Voided"){
	...
	} else if ($strPaymentStatus == "Refunded" && $strAuthStatus == "Completed"){
		
		/* 환불 완료(주문 배송 중일때 환불 요청을 받을때) */
		if ($orderRow[O_STATUS] != "C" && $orderRow[O_STATUS] != "R" && $orderRow[O_STATUS] != "T"){
			include MALL_HOME."/web/frwork/act/payCancel.php";	
			$strSuccess = "OK";
		}
	} else if ($strPaymentStatus == "Completed" && $strTxnType == "send_money"){
		
		/* 고객이 요청한 금액을 승인을 한 경우 */
		$orderMgr->setO_PAYPAL_RECEIVER_ID($strReceiverId);
		$intO_NO = $orderMgr->getOrderNo($db);
		$orderMgr->setO_NO($intO_NO);
		$orderRow = $orderMgr->getOrderView($db);
	...
	if(!file_exists(WEB_LOG_PATH."/".date("Y-m-d").".ipn_log.txt")) {
		$fp = fopen(WEB_LOG_PATH."/".date("Y-m-d").".ipn_log.txt","w");
	} else {
		$fp = fopen(WEB_LOG_PATH."/".date("Y-m-d").".ipn_log.txt","a");
	}
	flock( $fp, LOCK_EX );
	...
	fwrite($fp,"------------------------");
	fwrite($fp,"success===>".$strSuccess.":".$intO_NO);
	fwrite($fp,"\n\n\n");

	flock( $fp, LOCK_UN );
	fclose($fp);
    //mail('ivetmi@naver.com', 'Verified IPN', $listener->getTextReport());
    ...
    invalid IPN.
    */
    //mail('ivetmi@naver.com', 'Invalid IPN', $listener->getTextReport());
