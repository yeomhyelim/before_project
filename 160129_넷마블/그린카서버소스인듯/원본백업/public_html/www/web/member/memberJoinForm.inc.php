			<?if($strMenuType == "member"){?>

			<?if ($S_MEM_CERITY == "2"){?>
				<!-- 페이스북 회원가입 안내-->
				<div class="facebookSigninWrap">
					...
					<a href="javascript:goFacebookLogin();" class="facebookBtn"><span><img src="/himg/member/btn_login_facebook.gif"></span></a>
			...
			<!-- (1) 기본정보 입력  -->
				...
				<div class="regWrap">
					<table>
						...
						<!--회원구분-->
									<tr>
										<th><?=($S_JOIN_ID["NES"]=="Y")?"<strong>*</strong>":"<strong></strong>";?> <?= $LNG_TRANS_CHAR["MW00091"]; // 회원구분 ?></th>
										<td>
										<!--
										<?
										
										for($i=1; $i< sizeof($aryMemberGroup); $i++)
											{

										?>
										<input type="radio" id="memberGroup" name="memberGroup" class="defInput" maxlength="12" value='<?=$aryMemberGroup[$i][G_CODE]?>' onclick = "javascript:memberGradeCheck();" <?=($aryMemberGroup[$i][G_CODE] == '002') ? "checked" : "" ;?>/><?=$aryMemberGroup[$i][G_NAME];?>
										<?
											}
										?>
										-->
										<input type="radio" class="defInput" name="memberGroup" value="002" onclick = "javascript:memberGradeCheck();" checked/><span><?= $LNG_TRANS_CHAR["MW00092"]; //개인 ?></span>
										<input type="radio" class="defInput" name="memberGroup" value="005" onclick = "javascript:memberGradeCheck();"/><span><?= $LNG_TRANS_CHAR["MW00093"]; //사업자 ?></span>
										</td>
									</tr>
						<!--회원구분-->
						<!--아이디-->
										...
										<td>
											...
											<?if ($S_JOIN_MAILYN["USE"] == "Y" && $S_JOIN_MAILYN["JOIN"] == "Y"){?>
												<?if (!$S_JOIN_MAILYN["GRADE"] || in_array($strMemberJoinType,$S_JOIN_MAILYN["GRADE"])){?>
													<span class="txt_Minfo"><input type="checkbox" id="mailYN" name="mailYN" value="Y" checked/> <?=$LNG_TRANS_CHAR["MS00008"] //메일 정보를 수신합니다.?> </span>
												<?}?>
											<?}?>
											<span class="tdTextGuide"><?=$LNG_TRANS_CHAR["MS00051"]  //로그인 아이디는 이메일을 사용합니다. 사용하시는 이메일을 등록해 주세요. ?></span>
										</td>
									</tr>
								<?}?>
							<?}?>
						...
						<tr>
							...
							<td>
								<input type="text" id="nickname" name="nickname" class="defInput" maxlength="16"/> <a href="javascript:goNickNameChk();">[<?=$LNG_TRANS_CHAR["MW00066"]//중복체크?>]</a>
								<span class="tdTextGuide"><?=$LNG_TRANS_CHAR["MS00006"] //한글, 영문, 숫자 중 4자 이상 16자리 이하 사용?> </span>
							</td>
						</tr>
							<?}?>
						<?}?>
						...
						<tr>
							...
							<td>
								...
								<span class="txt_Minfo"><input type="checkbox" name="smsYN" id="smsYN" value="Y" checked/> <?=$LNG_TRANS_CHAR["MS00007"] //SMS 정보를 수신합니다.?> </span>
									<?}?>
								<?}?>
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								<dl>
									<dd><input type="text" id="zip1" name="zip1" class="defInput _w30" maxlength="3" readonly/> - <input type="text" id="zip2" name="zip2" class="defInput _w30" maxlength="3" readonly/> <a href="javascript:goZip(1);" class="btnIDChk">우편번호</a></a></dd>
						...
						<tr>
							<th><?=($S_JOIN_ADDR["NES"]=="Y")?"<strong>*</strong>":"<strong></strong>";?> <?=$LNG_TRANS_CHAR["MW00023"] //주?> </th>
							<td>
						...
						<tr>
							<th><?=($S_JOIN_PHOTO["NES"]=="Y")?"<strong>*</strong>":"<strong></strong>";?> <?=$LNG_TRANS_CHAR["MW00018"] //사진?></th>
							<td>
						...
						<tr>
							...
							<td>
								...
								<div style="padding: 3px 0;color:#55ac20">
									* 추천인의 <?=( $S_MEM_CERITY == 1 ? 'ID' : 'EMAIL' )?>을 정확히 입력해 주세요.
								</div>
							</td>
						</tr>
							<?}?>
				...
				<h4 class="joinTit2 mt30 comView" style="display:none;"><span><?=$LNG_TRANS_CHAR["MW00049"] //사업자 정보?></span></h4>
				<div class="regWrap comView" style="display:none;">
					<table>
						<colgroup>
							<col/>
							<col/>
						...
						<tr>
							...
							<td>
								<dl>
									<dd><input type="text" id="busi_zip1" name="busi_zip1" class="defInput _w30" maxlength="3" readonly value="<?=$aryMemBusiZip[0]?>"/> - <input type="text" id="busi_zip2" name="busi_zip2" class="defInput _w30" maxlength="3" readonly value="<?=$aryMemBusiZip[1]?>"/> <a href="javascript:goZip(3);" class="btnIDChk">우편번호</a></dd>
