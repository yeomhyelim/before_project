	...
	// 리스트 페이지로 이동
	function goCommunityBoardWriteListMoveEvent() {
	...
	// 커뮤니티 생성
	function goCommunityBoardWriteActEvent() {
		...
		if(!strBCode) {
			alert("게시판 코드가 없습니다.");
			objTarget.find('[name=b_code]').focus();
			return;
		}
		if(!strBName) {
			alert("게시판 이름은 필수 항목입니다.");
			 objTarget.find('[name=b_name]').focus();
			return;
		}
		...
		objTarget.find("input[name=act]").val("boardWrite");
		...
		$.ajax({
		   ...
		   ,success		: function(data) {	
								if(data['__STATE__'] == "SUCCESS") {
									...
									alert("생성되었습니다.");
									goCommunityBoardWriteListMoveEvent();
