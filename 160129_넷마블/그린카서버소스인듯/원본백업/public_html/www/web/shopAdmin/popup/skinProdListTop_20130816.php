<?
	...
	if(is_file($categoryPath)) { require_once "../conf/category.{$policyLng}.inc.php"; }
			...
			<table id="categoryList">
				...
				<tr id="category_<?=$level?>_<?=$key?>">
					...
					<td><?if($imgPath):?><img src="<?=$imgPath?>" style="width:100px;height:50px;"/>
						...
						<?if($tag):?>
						<textarea name="cateTag_<?=$key?>" id="cateTag_<?=$key?>" title="" style="width:100%;height:100px"><?include "{$S_DOCUMENT_ROOT}{$S_SHOP_HOME}{$cateWebTagPath}{$policyLng}/{$tag}";?></textarea>
