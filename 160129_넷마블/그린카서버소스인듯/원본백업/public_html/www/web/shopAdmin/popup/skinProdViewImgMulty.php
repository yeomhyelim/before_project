...
//		C_getAction("skinProdViewImgMulty","<?=$PHP_SELF?>");		

		doc.mode.value	= "act";
		doc.act.value	= "skinProdViewImgMulty";
...
//-->
	...
	<div class="adminTabBtnWrap mt10">
		...
		<a href="javascript:goPageMove('skinProdViewImgMulty')" class="selected" style="margin-left:200px">다중이미지</a>
						...
						<th>
							<div class="inBorderFFF">
								<ul>
									<li>네비게이션 사이즈를</li>
									<li>
										<strong>가로</strong> <input type="text" name="pv_img_multy_size_w" id="imgSizeW" value="<?=$row['PV_IMG_MULTY_SIZE_W']?>" <?=$nBox?>  style="width:25px;padding-right:5px;text-align:right;"/> px, 
										<strong>세로</strong> <input type="text" name="pv_img_multy_size_h" id="imgSizeH" value="<?=$row['PV_IMG_MULTY_SIZE_H']?>" <?=$nBox?>  style="width:25px;padding-right:5px;text-align:right;"/> px로 설정.</li>
								</ul>
							</div><!-- inBorderFFF -->
						</th>
						<th>
							<div class="inBorderFFF">
								<ul>
									...
									<li>
										<strong>가로</strong> 
										<select style="width:50px" name="pv_img_multy_view_w" id="imgViewW">
											<? for($i=1;$i<10;$i++) : ?>
											<option value="<?=$i?>"<?=($i==$row['PV_IMG_MULTY_VIEW_W'])?" selected":"";?>><?=$i?></option>
											<? endfor; ?>
										</select> 개, 
										<strong>세로</strong> 
										<select style="width:50px"  name="pv_img_multy_view_h" id="imgViewH">
											<? for($i=1;$i<15;$i++) : ?>
											<option value="<?=$i?>"<?=($i==$row['PV_IMG_MULTY_VIEW_H'])?" selected":"";?>><?=$i?></option>
											<? endfor; ?>
										</select> 줄로 설정.</li>
								</ul>
							</div><!-- inBorderFFF -->
					...
					<tr>
						...
						<td class="vTop">
							<div class="listBoxPreviewWrap">
								<div id="imageViewCnt" style="width:<?=$row['PV_IMG_MULTY_VIEW_W']*15?>px;height:<?=$row['PV_IMG_MULTY_VIEW_H']*15?>px;margin-top:-1px;margin-left:1px;background: url(./himg/layout/prod_list/list_dot_box.gif)"></div>
							</div>
						</td>
					</tr>
				</table>
				...
				<table>
					...
					<tr>
						<th class="pickerStyle">
							<div class="inBorderFFF">
								<strong class="">다중이미지에 마우스 오버시 디테일 이미지 벼환</strong>
								<strong class="block2">
									<span class="on_off"><input type="checkbox" value="Y" name="pv_img_multy_change"<?=($row['PV_IMG_MULTY_CHANGE']=="Y")?" checked":""?> id="on_off"/></span>
								</strong>
							</div><!-- inBorderFFF -->
						</th>
					</tr>
