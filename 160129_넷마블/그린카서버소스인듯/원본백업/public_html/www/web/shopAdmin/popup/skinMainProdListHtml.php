<?
	...
	$strTitImg = $row["{$strCode}_PRODLIST_TIT_".$strProdListOrder];
	
	$strProdImgSizeW = $row["{$strCode}_PRODLIST_IMG_SIZE_W_".$strProdListOrder];
	$strProdImgSizeH = $row["{$strCode}_PRODLIST_IMG_SIZE_H_".$strProdListOrder];
	$strProdImgViewW = $row["{$strCode}_PRODLIST_IMG_VIEW_W_".$strProdListOrder];
	$strProdImgViewH = $row["{$strCode}_PRODLIST_IMG_VIEW_H_".$strProdListOrder];
...
//		C_getAction("skinMainProdListHtml","<?=$PHP_SELF?>");

		doc.mode.value	= "act";
		doc.act.value	= "skinMainProdListHtml";
		var formData	= $("#form").serialize();

		C_AjaxPost("skinMainProdListHtmlAct", "./index.php", formData, "post");

	}

	function goAjaxRet(name,result){
		if (name == "skinMainProdListHtmlAct") {			
	...
	function goMainProdListTitImgDel(){
		...
		if (x == true)
		{
			...
			C_getAction("skinMainProdListTitImgDel","<?=$PHP_SELF?>");		
		}
	}
	...
	<form name="form" id="form">
		...
		<input type="hidden" name="prodListOrder" value="<?=$strProdListOrder?>">
			...
			<div class="buttonWrap">
				<a class="btn_blue_big" href="javascript:goAct();" id="menu_auth_w"><strong>설정 적용</strong></a>
