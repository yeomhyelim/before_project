<?
	...
	// 샵메인 정보 로드
	$designSetMgr->setDS_TYPE("SKIN_PJ");
	$skinRow			= $designSetMgr->getCodeView($db);
	$strSliderUseCss	= ($skinRow['PJ_SLIDER_USE']!="Y") ? "display:none" : "" ;

	for($i=1;$i<=5;$i++) :
		$skinRow['PJ_BEST_LIST'.$i.'_DESIGN'] = ($skinRow['PJ_BEST_LIST'.$i.'_DESIGN']) ? $skinRow['PJ_BEST_LIST'.$i.'_DESIGN'] : "PB0001"; 
	endfor;
	$skinRow['PJ_BEST_LIST1000_DESIGN'] = ($skinRow['PJ_BEST_LIST1000_DESIGN']) ? $skinRow['PJ_BEST_LIST1000_DESIGN'] : "PL0001";
?>
...
//		C_getAction("skin_pj_InfoSave", "<?=$PJP_SELF?>");
//		return;	

		var doc = document.form;
		doc.mode.value		= "act";
		doc.act.value		= "skin_pj_InfoSave";
		var formData		= $("#form").serialize();
		C_AjaxPost("skin_pj_InfoSave", "./index.php", formData, "post");
	}
...
//	}
	...
	function selfMainBestImgPopup(be_no, ds_code) {
		...
		var strUrl = './?menuType=popup&mode=skinProdBrandMainDesignSet&ic_type=sub&be_no='+be_no+'&ds_code='+ds_code+'&subPageCode=<?=$strSubPageCode?>';
...
//		var objPJ_TOP_USE_OP	= document.form.pl_top_use_op;
		var strType				= "A";
//		for(var i=0;i<intLen;i++){
//			if(objPJ_TOP_USE_OP[i].checked == true){
//				strType = objPJ_TOP_USE_OP[i].value;
//				break;
//			}
//		}
...
<!--
		...
		<? for($i=1;$i<=5;$i++): 
				$strCss = ($skinRow['PJ_BEST_LIST'.$i.'_NAME'] == "" || $skinRow['PJ_BEST_LIST'.$i.'_USE']!="Y") ? "display:none" : "";				?>
		<li id="pl_best_list<?=$i?>_use" style="<?=$strCss?>">
			<a href="javascript:goMainBestImg('<?=$i?>', '<?=$skinRow['PJ_BEST_LIST'.$i.'_DESIGN']?>')">
				<img src="http://www.eumshop.com/upload/shopDesign/subDesign/sub_prodlist_<?=$skinRow['PJ_BEST_LIST'.$i.'_DESIGN']?>.jpg" id="imgProdListDesign"/>
			</a>
		</li>
		<? endfor; ?>
-->
		<li id="pl_best_list1000_use"><a href="javascript:goMainBestImg('1', '<?=$skinRow['PJ_BEST_LIST1000_DESIGN']?>')" id="btnSkinProdListDesign">
			<img src="http://www.eumshop.com/upload/shopDesign/subDesign/sub_prodlist_<?=$skinRow['PJ_BEST_LIST1000_DESIGN']?>.jpg" id="imgProdListDesign" style="width:412px"/>
		</a></li>
	</ul>
</div>
<div class="layoutSettingInfo">
	...
	<ul>
		<li><input type="checkbox" name="pl_top_use_op_y" id="btnTopImageUse" value="A"<?if(in_array($skinRow['PJ_TOP_USE_OP'], array("A"))) { echo " checked"; }?>> 탑이미지 사용</li>
<!--
			<div class="subChk">
				<input type="radio" name="pl_top_use_op" value="A"<?=($skinRow['PJ_TOP_USE_OP']=="A") ? " checked" : "" ?>> 모든 상품페이지에 한개의 이미지만 적용<br>	
				<input type="radio" name="pl_top_use_op" value="B"<?=($skinRow['PJ_TOP_USE_OP']=="B") ? " checked" : "" ?>> 카테고리별 이미지 업로드 적용					
			</div>


		<li><input type="checkbox" name="pl_sub_cate_use" id="btnSubCategoryUse" value = "Y"<?=($skinRow['PJ_SUB_CATE_USE']=="Y") ? " checked" : "" ?>> 서브 카테고리 사용</li>

		<div id="displayUseArea">
			<? for($i=1;$i<=5;$i++): 
					$strCss = ($skinRow['PJ_BEST_LIST'.$i.'_NAME'] == "") ? "display:none" : "";				?>
			<li style="<?=$strCss?>">
				<input	type="checkbox" name="pl_best_list<?=$i?>_use" id="pl_best_list<?=$i?>_use" value="Y" "<?=($skinRow['PJ_BEST_LIST'.$i.'_USE']=="Y") ? " checked" : "" ?>/><?=$skinRow['PJ_BEST_LIST'.$i.'_NAME']?>
			</li>
			<? endfor; ?>
		</div>
