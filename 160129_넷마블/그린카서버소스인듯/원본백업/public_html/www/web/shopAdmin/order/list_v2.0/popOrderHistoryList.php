	...
	<div class="tableList" style="margin-top:10px;">
		<table>
					...
					for($i=0;$i<sizeof($aryOrderHistoryList);$i++){
						...
						if ($aryOrderHistoryList[$i]['H_CODE'] == "002"){
							...
							if ($aryOrderHistoryList[$i]['H_TEXT']){
								...
								$shopOrderCartRow	= $shopOrderMgr->getOrderCartList($db,"OP_SELECT",$param);
							...
							if ($shopOrderCartRow['M_NO'] == $aryOrderHistoryList[$i]['M_NO']) $shopOrderCartRow['ST_NAME'] = "";
						}
			...
			<tr>
				...
				<td><?=$shopOrderCartRow['ST_NAME']?><?=($strHistoryText)?"[".$strHistoryText."]":"";?></td>
