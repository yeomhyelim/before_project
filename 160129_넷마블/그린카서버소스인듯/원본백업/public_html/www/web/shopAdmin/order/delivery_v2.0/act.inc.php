	...
	switch ($strAct) {
		...
		case "orderDeliveryExcelUpdate":
			...
			foreach($data->sheets[0]['cells'] as $key => $data):
				...
				if(!$data[$cellDeliveryCom] || !$data[$cellDeliveryNum]) { $strDeliveryStatus = "B"; continue;} // 배송준비중

				## order_cart 배송상태 확인(배송상태가 배송중이면 다시 update 하지 않음)
				$param							= "";
				$param["OC_NO"]					= $data[$cellCode];
				$param["O_SHOP_NO"]				= -1;
				$param["O_USE_LNG"]				= $S_ST_LNG;
				$orderCartRow					= $shopOrderMgr->getOrderCartList($db,"OP_SELECT",$param);
				
				if (!$orderCartRow) {continue;}
				if ($orderCartRow['OC_DELIVERY_STATUS'] == "I" && $orderCartRow['OC_DELIVERY_NUM'] == $strDeliveryNum && $orderCartRow['OC_DELIVERY_COM'] == $aryDeliveryComName[$strDeliveryCom]) {continue;}
				
				## 배송회사가 존재하지 않을 경우
				if (!$aryDeliveryComName[$strDeliveryCom]){
					$strProdCancelMsg		   .= "[".$orderCartRow['P_CODE']."] 상품(번호:".$orderCartRow['OC_NO'].")의 배송회사는 설정되어 있는 배송회사가 아닙니다.(입접사정보에서 배송회사를 추가하세요.)\\n";
					continue;
				}
				
				## 취소상품 업로드 불가
				if (substr($orderCartRow['OC_ORDER_STATUS'],0,1) == "C"){
					$strProdCancelMsg		   .= "[".$orderCartRow['P_CODE']."] 상품은 취소된 상품입니다.\\n";
					continue;
				}
				...
				$intOC_NO						= $param["OC_NO"];
				$param["O_NO"]					= $intO_NO;

				## 배송정보 UPDATE HISTORY 남기기
				$strOrderCartStatusMemo			= "배송상태변경(엑셀)";
				$strOrderCartStatusText			= $param["OC_NO"]."/".$strDeliveryStatus;
				
				$historyParam					= "";
				$historyParam['m_no']			= $a_admin_no;
				$historyParam['h_tab']			= TBL_ORDER_MGR;
				$historyParam['h_key']			= $intO_NO;
				$historyParam['h_code']			= "003"; //배송상태
				$historyParam['h_memo']			= $strOrderCartStatusMemo;
				$historyParam['h_text']			= $strOrderCartStatusText;
			...
			$strUrl = $_SERVER['HTTP_REFERER'];
			$strMsg	= $strProdCancelMsg."송장엑셀파일 데이터가 일괄수정 되었습니다.";
