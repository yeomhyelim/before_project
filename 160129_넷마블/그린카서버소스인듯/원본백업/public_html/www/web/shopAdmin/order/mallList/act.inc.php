...
//		endif;
	...
	$strSearchDeliveryStatus= $_POST["searchDeliveryStatus"]	? $_POST["searchDeliveryStatus"]	: $_REQUEST["searchDeliveryStatus"];
	...
	$strLinkPage .= "&searchMemberType=$strSearchMemberType&searchDeliveryCom=$strSearchDeliveryCom&searchDeliveryStatus=$strSearchDeliveryStatus";
...
//	$strLinkPage .= "&searchSettleT=$strSearchSettleT&searchSettleB=$strSearchSettleB";
	...
	if ($S_FIX_MEMBER_CATE_USE_YN  == "Y"){
		$strLinkPlage .= "&searchNation=$strSearchNation";
		$strLinkPlage .= "&searchCate1=$strSearchCate1";
		$strLinkPlage .= "&searchCate2=$strSearchCate2";
		$strLinkPlage .= "&searchCate3=$strSearchCate3";
		$strLinkPlage .= "&searchCate4=$strSearchCate4";
	}
...
//	endif;
	...
	switch ($strAct) {
		case "orderBuyAccept":
			// 2013.06.26 kim hee sung, 배송완료목록에서 구매완료 처리.
			
			## STEP 1.
			## 배열값 체크
			$aryChkNo	= $_POST['chkNo'];
			if(!is_array($aryChkNo)):
				goErrMsg("선택된 항목이 없습니다.");
				exit;
			endif;
			...
			foreach($aryChkNo as $key => $so_no):
				...
				$param				= "";
				$param['so_no']		= $so_no;
				$shopOrderRow		= $shopOrderMgr->getShopOrderListEx($db, "OP_SELECT", $param);
				...
				$orderCartResult	= $orderMgr->getOrderCartListEx($db, "OP_LIST", $param);
				
				while($row = mysql_fetch_array($orderCartResult)):
						$param							= "";
						$param['oc_no']					= $row['OC_NO'];
						$param["oc_status1"]			= "E";
						$param["oc_status2"]			= "";
						$param["oc_req_dt"]				= "";
						$param["oc_reg_dt"]				= "";
		...
		case "orderCancel":
			...
			if($strMsg) :
				...
				if ($S_MALL_TYPE == "M"){
					...
					$shopOrderMgr->getOrderStatusAllProcessUpdate($db,$param);
				}
				/* 몰인몰형태일때 몰인몰 관련 데이터 취소로 변경(2013.06.27) */
...
//			endif;
		...
		case "orderStatusSave":
			...
			## STEP 1.
			## 구매상태 정보 NO 값 설정				
			$aryOC_NO = $_POST['chkNo'];
			if(!is_array($aryOC_NO)):
				goErrMsg($LNG_TRANS_CHAR["OS00022"]); //"변경할 주문이 존재하지 않습니다."
				exit;
			endif;
			...
			foreach($aryOC_NO as $oc_no):
				$param['oc_no']					= $oc_no;
				$param['oc_order_status']		= $_POST["orderStatus_{$oc_no}"];
				...
				if ($param['oc_order_status']){
					...
					if($re != 1):
						goErrMsg($LNG_TRANS_CHAR["OS00023"]."({$so_no})"); //"구매상태 업데이트 실패 건이있습니다.({$so_no})"
						exit;
					endif;
					
					/* 전체 주문 상태 update(마스터 주문상태/샵 주문상태 변경) */
					$param['so_no']					= "";
					$param['update_type']			= "P";
			...
			//$strUrl = "./?menuType=order&mode=list&".$strLinkPage;
			$strUrl = $_SERVER['HTTP_REFERER'];
		...
		case "deliverySave":
			...
			if(!is_array($arySO_NO)):
				goErrMsg($LNG_TRANS_CHAR["OS00022"]);
				exit;
			endif;
			...
			foreach($arySO_NO as $so_no):
				...
				if (!$param['so_delivery_status'] && $param['so_delivery_com'] && $param['so_delivery_num']) $param['so_delivery_status'] = "I";

				$re								= $shopMgr->getDeliveryUpdateEx($db, $param);
				if($re != 1):
					echo $LNG_TRANS_CHAR["OS00024"]."({$so_no})"; //배송 정보 업데이트 실패 건이있습니다.
					exit;
				endif;
