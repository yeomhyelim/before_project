<?
	...
	$count=1;
	$intFileCheck=0;
	if($shopRow['SH_COM_CERTIFICATES1_FILE']) {	$sh_certificates1	= "/upload/shop/certificates1/{$shopRow['SH_COM_CERTIFICATES1_FILE']}"; $count=1;$intFileCheck=1;}
	if($shopRow['SH_COM_CERTIFICATES2_FILE']) {	$sh_certificates2	= "/upload/shop/certificates2/{$shopRow['SH_COM_CERTIFICATES2_FILE']}"; $count=2;$intFileCheck=2;}
	if($shopRow['SH_COM_CERTIFICATES3_FILE']) {	$sh_certificates3	= "/upload/shop/certificates3/{$shopRow['SH_COM_CERTIFICATES3_FILE']}"; $count=3;$intFileCheck=3;}
	if($shopRow['SH_COM_CERTIFICATES4_FILE']) {	$sh_certificates4	= "/upload/shop/certificates4/{$shopRow['SH_COM_CERTIFICATES4_FILE']}"; $count=4;$intFileCheck=4;}
	if($shopRow['SH_COM_CERTIFICATES5_FILE']) {	$sh_certificates5	= "/upload/shop/certificates5/{$shopRow['SH_COM_CERTIFICATES5_FILE']}"; $count=5;$intFileCheck=5;}
	...
	<div class="tableFormWrap">
		...
		<table class="tableForm">
			<tr>
				...
				<?endif;?>
				<?if($a_admin_type == "S"): ?>
					<input type="hidden" style="width:90%;" id="shop_main" name="shop_main" value="<?=$shopRow[SH_COM_MAIN]?>"/>
				<?endif;?>
			</tr>
			<tr>
			...
			<tr>
				...
				<td colspan="3">
					<div id="divAddr1" class="divAddr1" <?=($shopRow[SH_COM_COUNTRY] != "KR") ? " style=\"display:none\" ":"";?>>
					...
					<div id="divAddr2" class="divAddr2" <?=($shopRow[SH_COM_COUNTRY]=="KR")?"style=\"display:none\"":"";?>>
						...
						<div id="divState1" class="divState1" <?=($shopRow[SH_COM_COUNTRY]=="US")?"style=\"display:none\"":"";?>>
						...
						<div id="divState2" class="divState2" <?=($shopRow[SH_COM_COUNTRY]!="US")?"style=\"display:none\"":"";?>>
		...
		<table class="tableForm">
			...
			<tr>
				...
				<td colspan="3" id="dynamic_table">
					<?
					if($intFileCheck != 0){
						for($i=1; $i <= $intFileCheck; $i++){
					?>
						
						<div>
						<input type="text" id="com_certificates<?=$i?>" name="com_certificates<?=$i?>" value="<?=$shopRow["SH_COM_CERTIFICATES{$i}"];?>">
						<input type="file" <?=$nBox?>  id="com_certificates<?=$i?>_file" name="com_certificates<?=$i?>_file"/>
						<?
						//echo ${"sh_certificates".$i};
						if( ${"sh_certificates".$i}){
						?>
						<!--<a href="<?= ${"sh_certificates".$i}?>"><img src="<?= ${"sh_certificates".$i}?>" style="width:70px;height:70px"></a>-->
						<a class="btn_sml" href="<?= ${"sh_certificates".$i}?>" ><span>파일보기</span></a>
