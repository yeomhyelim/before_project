...
//		if (!is_array($aryOrderCartList)) continue;
		...
		if ($aryOrderCartList[0]['OC_BAESONG_TYPE'] == "1") $strCartDeliveryPayMthName = "(주문시결제)";
		else if ($aryOrderCartList[0]['OC_BAESONG_TYPE'] == "2") $strCartDeliveryPayMthName = "(착불)";
	...
	<tr>
		...
		<td><?=$row['O_B_HP']?></td>
		<td><?=$row['O_B_MAIL']?></td>
		<td>[<?=$row['O_B_ZIP']?>] <?=$row['O_B_ADDR1']?> <?=$row['O_B_ADDR2']?></td>
		<td><?=$row['O_B_MEMO']?></td>
		...
		<td><?=$row['O_APPR_DT']?></td>
		<td><?=$strOrderSettleStatus?></td>
		<td><?=$strOrderStatus?></td>
	...
	<tr>
		...
		<?if ($S_MALL_TYPE != "R"){?>
		<td><?=$aryOrderCartList[$j][P_SHOP_NO]?> : <?=$aryOrderCartShopList[$aryOrderCartList[$j][P_SHOP_NO]]['ST_NAME']?></td>
		<?}?>
