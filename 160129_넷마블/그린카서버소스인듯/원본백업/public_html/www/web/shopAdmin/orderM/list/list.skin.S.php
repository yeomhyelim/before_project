		...
		<table style="border-left:1px solid #D2D0D0">
			...
			<?}else{
				while($row = mysql_fetch_array($orderListResult)){
					...
					$strOrderMemberId = "";
					if ($S_MEM_CERITY == "1") {
						$strOrderMemberId = "<li>(".$row['M_ID'].")</li>";
					}
			...
			<tr>
				...
				<td <?=$strCartRowSpanHtml?>><?=$strOrderSettleStatusText?></td>
				<?
				...
				<td>
					...
					<ul id="order_cart_delivery_<?=$row['O_NO']?>_<?=$aryOrderCartList[$i]['OC_NO']?>">
						...
						<li>
							<a class="btn_sml" id="btnCartDeliverySave" href="javascript:goOrderCartDeliveryUpdate(<?=$row['O_NO']?>,'<?=$aryOrderCartList[$i]['OC_NO']?>','<?=$aryOrderCartList[$i]['P_SHOP_NO']?>');"><strong><?="개별배송정보변경" //개별배송정보변경?></strong></a>
						</li>
					</ul>
				</td>
				<td>
					<div id="order_cart_order_<?=$aryOrderCartList[$i]['OC_NO']?>">
						<ul>
							...
							<?
								if ($aryOrderCartList[$i]['OC_ORDER_STATUS'] == "C"){
			...
			<tr>
				...
				<td colspan="11" style="text-align:left;">
					<ul>
						...
						<li>	
							<?=$LNG_TRANS_CHAR["OW00034"]//배송지정보?> : [<?=$row[O_B_NAME]?>] [<?=$row[O_B_ZIP]?>] <?=$row[O_B_ADDR1]?> <?=$row[O_B_ADDR2]?> <?=$row[O_B_CITY]?> <?=$strDeliveyState?> <?=$aryCountryList[$row[O_B_COUNTRY]]?> <a class="btn_sml" href="javascript:goOrderDeliveryAddr('<?=$row['O_NO']?>');"><strong>배송정보수정</strong></a>
						</li>
					</ul>
				</td>
			</tr>
