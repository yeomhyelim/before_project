...
//						$intAccPrice	= getCurToPrice($row['P_SALE_PRICE'] - $row['P_STOCK_PRICE'],$S_ST_LNG);
			...
			<tr>
				...
				<td><input type="input" name="point_<?=$strProdCode?>" id="point_<?=$strProdCode?>" value="<?=$intPoint?>" <?=$priceBox?> style="width:50px;"/> P</td>
				<td><input type="input" name="qty_<?=$strProdCode?>" id="qty_<?=$strProdCode?>" value="<?=$strProdQty?>" <?=$nBox?> style="width:40px;text-align:right;"<?=$strProdQtyReadOnly?>/></td>
				<td><?=$strProdRepDt?></td>
				<td>
