<?
	...
	## 2.11 상품재고 
	$strProdStockOut					= $_REQUEST['searchStock1'];
	$strProdStockLimit					= $_REQUEST['searchStock2'];
	$strProdReStock						= $_REQUEST['searchStock3'];
	
	$param['P_STOCK_OUT']				= $strProdStockOut;
	$param['P_STOCK_LIMIT']				= $strProdStockLimit;
	$param['P_RESTOCK']					= $strProdReStock;

	## 2.12 다국어출력사용여부(리스트)
	$aryUseLng							= explode("/", $S_USE_LNG);
	$intUseLngCount						= count($aryUseLng);
	$param['P_USE_LNG']					= $aryUseLng;
	$param['P_MANY_LNG_VIEW_SHOW']		= "Y";
