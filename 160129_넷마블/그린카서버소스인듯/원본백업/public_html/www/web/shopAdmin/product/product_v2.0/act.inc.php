...
//	$strProdLng				= $_POST["prodLang"]				? $_POST["prodLang"]				: $_REQUEST["prodLang"];
	...
	if ($a_admin_type == "S") $productMgr->setP_SHOP_NO($a_admin_shop_no);
	...
	//					$copyRe				= copy("{$S_DOCUMENT_ROOT}{$S_SHOP_HOME}{$url1}", "{$S_DOCUMENT_ROOT}{$S_SHOP_HOME}/{$url2}");
			...
			if($S_PRODUCT_SEARCH_USE == "Y"):
				...
				if($aryProductSearch):
					foreach($aryProductSearch as $psKey => $psData):
						...
						if($aryProductSearchData[$strCommGrp]) { $aryProductSearchData[$strCommGrp] .=","; }
			...
			$strUrl = "./?menuType=".$strMenuType."&mode=".$strMode."&".$strLinkPage;
		...
		case "prodModify":
			...
			if($S_PRODUCT_SEARCH_USE == "Y"):
				...
				$param['PS_PROD_SEARCH_9']		= $aryProductSearchData['PROD_SEARCH_9'];
				$param['PS_PROD_SEARCH_10']		= $aryProductSearchData['PROD_SEARCH_10'];
				if($intProductSearchCnt) {  $objProductSearchModule->getProductSearchUpdateEx($param); }
				else { $objProductSearchModule->getProductSearchInsertEx($param); }
			endif;
