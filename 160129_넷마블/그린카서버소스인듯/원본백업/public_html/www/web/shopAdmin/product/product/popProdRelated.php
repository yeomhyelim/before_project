	...
	$(document).ready(function(){
		...
		if(obj) {
			...
			for(var i = 0; i < intObjLength; i++) {
				...
				// 기본설정
				var objTarget = $("#proChooseArea");
				var strPM_REAL_NAME = obj[i]['PM_REAL_NAME'];
				var strP_BRAND = obj[i]['P_BRAND'];
				var strP_CODE = obj[i]['P_CODE'];
				var strP_NAME = obj[i]['P_NAME'];
				var strP_SALE_PRICE = obj[i]['P_SALE_PRICE'];

				var strHtmlTemp = '<dl id="p_code_' + strP_CODE + '" class="prodlist">\n' + 
										'<dt>\n' + 
											'<input type="checkbox" name="prodSelect" id="prodSelect" value="' + strP_CODE + '">\n' + 
											'<img id="pm_real_name" src="' + strPM_REAL_NAME + '" style="width:70px;height:70px">\n' + 
										'</dt>\n' + 
										'<dd id="p_code">' + strP_CODE + '</dd>\n' + 
										'<dd id="p_brand"></dd>\n' + 
										'<dd id="p_name">' + strP_NAME + '</dd>\n' + 
										'<dd id="p_sale_price">' + strP_SALE_PRICE + '</dd>\n' + 
									'</dl>\n';

				strHtml = strHtml + strHtmlTemp;
			}

			objTarget.html(strHtml);
		}

// 2014.07.30 kim hee sung, have the bug
//		var obj = parent.getProdRelatedList(); 
//		if(obj){
//			var len				= obj.length;
