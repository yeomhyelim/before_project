...
?>
	<table>
		...
		<?if(in_array($b_kind_skin, array("data_basic", "data_blog", "data_gallery"))):?>
		<tr>
			<th>게시판종류</th>
			<td>
				<ul class="designType">
					<li>
						...
						<span><input type="radio" name="b_kind_skin" id="b_kind_skin" value="data_basic"<?if($b_kind_skin=="data_basic"){ echo " checked"; }?>/>일반게시판</span>
					</li>
					<li>
						<img src="/shopAdmin/himg/layout/sample/board_type_data_gallery.gif"/>
						<span><input type="radio" name="b_kind_skin" id="b_kind_skin" value="data_gallery"<?if($b_kind_skin=="data_gallery"){ echo " checked"; }?>/>갤러리형</span>
					</li>
					<li>
						<img src="/shopAdmin/himg/layout/sample/board_type_data_blog.gif"/>
						<span><input type="radio" name="b_kind_skin" id="b_kind_skin" value="data_blog"<?if($b_kind_skin=="data_blog"){ echo " checked"; }?>/>블러그형</span>
					</li>
				</ul>
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				...
				<span class="listCntColumn<?php echo $strListCntColumnHide;?>"><input type="input" name="bi_column_default" value="<?=$boardInfoAry['BI_COLUMN_DEFAULT']?>" class="_w50"/> 칸</span>
				<span class="listCntLine"><input type="input" name="bi_list_default" class="_w50" value="<?=$boardInfoAry['BI_LIST_DEFAULT']?>"/> 라인</span>
			</td>
		</tr>
		<tr>
			<th>타이틀 표시방법</th>
		...
		<tr>
			...
			<td><input type="radio" name="bi_start_mode" value="dataList"<?if($boardInfoAry['BI_START_MODE']=="dataList"){echo " checked";}?>/>목록화면	
				...
				<!--input type="radio" name="bi_start_mode" value="dataView"<?if($boardInfoAry['BI_START_MODE']=="dataView"){echo " checked";}?>/>상세보기화면--><? ## 2013.04.29 이사님 요청으로 삭제 ?></td>
		</tr>
		<tr>
			<th>글쓰기후이동</th>
