	...
	<table class="tableForm">
		<tr>
			...
			<td>
				...
				<input type="text" <?=$nBox?>  style="width:300px;" id="ub_name" name="ub_name" value="<?=$dataSelectRow['UB_NAME']?>" check="Y" maxlength="10"/>
				<?else:								// 비회원이 작성한 글		?>
				<input type="text" <?=$nBox?>  style="width:300px;" id="ub_name" name="ub_name" value="<?=$dataSelectRow['UB_NAME']?>" check="Y" maxlength="10"/>
				<?endif;?>
			</td>
		...
		<tr>
			...
			<td>
				...
				<input type="text" <?=$nBox?>  style="width:300px;" id="ub_mail" name="ub_mail" value="<?=$dataSelectRow['UB_MAIL']?>" check="Y" maxlength="30"/>
				<? else:							// 비회원 ?>
				<input type="text" <?=$nBox?>  style="width:300px;" id="ub_mail" name="ub_mail" value="<?=$dataSelectRow['UB_MAIL']?>" check="Y" maxlength="30"/>
				<? endif; ?>
			</td>
		...
		<tr>
			...
			<td>
				<input type="text" <?=$nBox?> id="ub_title" name="ub_title" value="<?=stripslashes($dataSelectRow['UB_TITLE'])?>" check="Y" maxlength="50" style="width:98%;"/>
				<?if($_REQUEST['BOARD_INFO']['bi_datawrite_notice_use']=="Y" && $_REQUEST['member_group']=="001"):?>
				<span class="boardTxtInfo">
				<input type="checkbox" id="ub_func_notice" name="ub_func_notice" value="Y"<?if($dataSelectRow['UB_FUNC']['UB_FUNC_NOTICE']=="Y"){echo " checked";}?>/> <?=$LNG_TRANS_CHAR["CW00068"] //공지글?>
				</span>
				<?endif;?>
				<?if($_REQUEST['BOARD_INFO']['bi_widget_icon_use']=="Y"):?>
					<span class="boardTxtInfo">
						<input type="checkbox" id="ub_func_icon_widget" name="ub_func_icon_widget" value="Y"<?if($dataSelectRow['UB_FUNC']['UB_FUNC_ICON_WIDGET']=="Y"){echo " checked";}?>/> <?=$LNG_TRANS_CHAR["CW00069"] //위젯글?>
					</span>
				<?endif;?>
				<?if($_REQUEST['BOARD_INFO']['bi_datawrite_lock_use']=="C"):?>
				<span class="boardTxtInfo">
		...
		<tr>
			...
			<td>
				<textarea style="width:100%;height:300px" id="ub_text" name="ub_text" title="<?=$strDataWriteForm?>" check="Y"><?=stripslashes($dataSelectRow['UB_TEXT'])?></textarea>
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				<a class="btn_sml" href="javascript:goAttachedfileOpen();" id="menu_auth_w" style=""><strong><?=$LNG_TRANS_CHAR["CW00058"] //첨부파일?></strong></a>
				<div id="fileList">
