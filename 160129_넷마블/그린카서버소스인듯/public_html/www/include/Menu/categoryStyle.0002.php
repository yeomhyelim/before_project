<?
	## STEP 1.
	## 설정
	$category_use		= $S_MAIN_SUB_CATE_USE2; // 사용유무
	$category_1_use		= $S_MAIN_CATE_L1_MODE2; // 카테고리 레벨1 사용 유무
	$category_2_use		= $S_MAIN_CATE_L2_MODE2; // 카테고리 레벨2 사용 유무
	$category_3_use		= $S_MAIN_CATE_L3_MODE2; // 카테고리 레벨3 사용 유무
	$category_4_use		= $S_MAIN_CATE_L4_MODE2; // 카테고리 레벨4 사용 유무

	$category_4_keep	= "Y";	// 유지 
//	if($category_4_keep == "Y") { $strSearchHCode4 ="";

	if(!$strSearchHCode1) { $strSearchHCode1 = "001"; }
?>

<? if($category_use == "Y"): ?>
<? $hrefFmt		= "./?menuType=product&mode=list&lcate=%s&mcate=%s&scate=%s&fcate=%s"; ?>
<? for($i = 0;$i < sizeof($S_ARY_CATE1); $i++):	// 카테고리1?>
<? if($strSearchHCode1 && $S_ARY_CATE1[$i]['CODE'] != $strSearchHCode1) { continue; }  ?>
<? $href		= sprintf($hrefFmt, $S_ARY_CATE1[$i]['CODE'], "", "", ""); ?>
<? $className1	= ($strSearchHCode1 ==  $S_ARY_CATE1[$i]['CODE']) ? "selectedNavi" : ""; ?>
<ul class="cate1">
	<? if($category_1_use == "T") : ?>
		<!-- html -->
	...
	<!--li-->
		...
		<? if($strSearchHCode2 && $S_ARY_CATE2[$i][$j]['CODE'] != $strSearchHCode1.$strSearchHCode2) { continue; }  ?>
		...
		<? $className2	= ($strSearchHCode1.$strSearchHCode2 ==  $S_ARY_CATE2[$i][$j]['CODE']) ? "selectedNavi" : ""; ?>
			<? if($category_2_use == "T") : ?>
					<!-- html -->
			...
			<!--li-->
				...
				<? if($strSearchHCode3 && $S_ARY_CATE3[$i][$j][$k]['CODE'] != $strSearchHCode1.$strSearchHCode2.$strSearchHCode3) { continue; }  ?>
				...
				<? $className3	= ($strSearchHCode1.$strSearchHCode2.$strSearchHCode3 ==  $S_ARY_CATE3[$i][$j][$k]['CODE']) ? "selectedNavi" : ""; ?>
					<? if($category_3_use == "T") : ?>
						<!-- html -->
					...
					<!--li-->
						...
						<? if($strSearchHCode4 && $S_ARY_CATE4[$i][$j][$k][$l]['CODE'] != $strSearchHCode1.$strSearchHCode2.$strSearchHCode3.$strSearchHCode4) { continue; }  ?>
						...
						<? $className4	= ($strSearchHCode1.$strSearchHCode2.$strSearchHCode3.$strSearchHCode4 ==  $S_ARY_CATE4[$i][$j][$k][$l]['CODE']) ? "selectedNavi" : ""; ?>
							<? if($category_4_use == "T") : ?>
								<!-- html -->
