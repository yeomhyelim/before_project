		...
		<dl>
			...
			<dd class="detailInfo">
				<table>
					...
					<?if ($prodRow[P_CONSUMER_PRICE] > 0){?>
					<tr>
						<th><?=$LNG_TRANS_CHAR["PW00002"] //소비자가?></th>
						<td><?=$strMoneyIconL?><s class="priceOrg"><?=getCurToPrice($prodRow[P_CONSUMER_PRICE])?><?=$strMoneyIconR?></s></td>
					</tr>
					<?}?>
					<tr>
						<th><?if($S_PRODUCT_RENT == "Y" && $prodRow[P_STOCK_PRICE] != "1" ){echo $LNG_TRANS_CHAR["PW00003"]; }else{echo $LNG_TRANS_CHAR["PW00004"];} //임대가/판매가?></th>
						<td>
							<?if ($prodRow[P_EVENT_UNIT] && $prodRow[P_EVENT]){?><s><?}?>
								 <?=$strMoneyIconL?><strong class="priceOrange _fs14" id="realPayPriceText"> <?=getProdDiscountPrice($prodRow)?></strong><?=$strMoneyIconR?>
							<?if ($prodRow[P_EVENT_UNIT] && $prodRow[P_EVENT]){?></s><?}?>
						</td>
					</tr>
					<?if ($prodRow[P_EVENT_UNIT] && $prodRow[P_EVENT]){?>
						<tr>
							...
							<td> <?=$strMoneyIconL?><strong class="priceOrange _fs14" id="realPayPriceTaxText"><?=getCurToPrice(getProdEventPrice($prodRow[P_SALE_PRICE],$prodRow[P_EVENT_UNIT],$prodRow[P_EVENT]))?></strong> <?=$LNG_TRANS_CHAR["PW00007"] //(10%할인)?><?=$strMoneyIconR?>
							</td>
						</tr>
					<?}?>
					<?if ($intProdPoint > 0){?>
					<tr>
						...
						<td><?=getCurToPrice($intProdPoint)?> <?=$LNG_TRANS_CHAR["PW00008"] //포인트?></td>
					</tr>	
					<?}?>				
				...
				<?
					...
					<tr>
						...
						<strong class="btnCntUpDown">
							<a href="javascript:goProdViewQtyChange('up',<?=$prodRow[P_MIN_QTY]?>);"><img src="../himg/product/A0001/btn_prod_cnt_up.gif"/></a>
							<a href="javascript:goProdViewQtyChange('down',<?=$prodRow[P_MIN_QTY]?>);"><img src="../himg/product/A0001/btn_prod_cnt_down.gif"/></a>
						</strong>
						<div class="clear"></div>
						</td>
					</tr>
				...
				<div class="orderBtnWrap mt40">
					<a href="javascript:goCartOrder();" class="btnProdBuy"><span><?=$LNG_TRANS_CHAR["PW00021"] //상품구매?></span></a>
					<a href="javascript:goCart();" class="btnProdCart"><span><?=$LNG_TRANS_CHAR["PW00022"] //장바구니?></span></a>
					<a href="javascript:goWish();" class="btnProdWish"><span><?=$LNG_TRANS_CHAR["PW00023"] //담아두기?></span></a>
