...
<!--
	...
	function goExcelDownload() {
		...
		data['mode']				= "excel";
		data['act']					= "excelOrderDayStatics2";
...
<div id="contentArea">
	<div class="contentTop">
		<h2><?= $LNG_TRANS_CHAR["EW00142"]; //일별주문통계 ?></h2>
		<div class="clr"></div>
	</div>
	...
	<div class="tableList mt30">
		<table>
			...
			<tr>
				<th rowspan="2"><?= $LNG_TRANS_CHAR["EW00143"]; //일자 ?></th>
				<th rowspan="2"><?= $LNG_TRANS_CHAR["EW00144"]; //총매출액 ?></th>
				<th colspan="2"><?= $LNG_TRANS_CHAR["EW00145"]; //주문완료(결제대기건) ?></th>
				<th colspan="2"><?= $LNG_TRANS_CHAR["EW00148"]; //결제완료 ?></th>
				<th colspan="2"><?= $LNG_TRANS_CHAR["EW00149"]; //배송준비중 ?></th>
				<th colspan="2"><?= $LNG_TRANS_CHAR["EW00150"]; //배송중 ?></th>
				<th colspan="2"><?= $LNG_TRANS_CHAR["EW00151"]; //배송완료 ?></th>
				<th colspan="2"><?= $LNG_TRANS_CHAR["EW00152"]; //구매완료 ?></th>
				<th colspan="2"><?= $LNG_TRANS_CHAR["EW00153"]; //주문취소 ?></th>

			</tr>
			<tr>
				

				<th><?= $LNG_TRANS_CHAR["EW00146"]; //건수 ?></th>
				<th><?= $LNG_TRANS_CHAR["EW00147"]; //금액 ?></th>
			...
			if (is_array($aryOrderSaleList)){
				...
				for($i=0;$i<sizeof($aryOrderSaleList);$i++){
					...
					$strUrl							= "./?menuType=order&searchRegStartDt=".$aryOrderSaleList[$i][O_REG_DT]."&searchRegEndDt=".$aryOrderSaleList[$i][O_REG_DT];
					
					?>
			<tr>
				<td class="txtCenter"><?=$aryOrderSaleList[$i][O_REG_DT]?></td>

				<td><?=getFormatPrice($intOrderTotalPrice,2,$S_ST_CUR)?></td>
				
				<td><a href="<?=$strUrl?>&mode=list&searchOrderStatus=J" target="_blank"><?=NUMBER_FORMAT($aryOrderSaleList[$i][O_STATUS_CNT1])?></a></td>
				<td><?=getFormatPrice($aryOrderSaleList[$i][O_STATUS_PRICE1],2,$S_ST_CUR)?></td>

				<td><a href="<?=$strUrl?>&mode=list&searchOrderStatus=A" target="_blank"><?=NUMBER_FORMAT($aryOrderSaleList[$i][O_STATUS_CNT2])?></a></td>
				<td><?=getFormatPrice($aryOrderSaleList[$i][O_STATUS_PRICE2],2,$S_ST_CUR)?></td>

				<td><a href="<?=$strUrl?>&mode=deliveryList&searchOrderStatus=B" target="_blank"><?=NUMBER_FORMAT($aryOrderSaleList[$i][O_STATUS_CNT3])?></a></td>
				<td><?=getFormatPrice($aryOrderSaleList[$i][O_STATUS_PRICE3],2,$S_ST_CUR)?></td>

				<td><a href="<?=$strUrl?>&mode=deliveryList&searchOrderStatus=I" target="_blank"><?=NUMBER_FORMAT($aryOrderSaleList[$i][O_STATUS_CNT4])?></a></td>
				<td><?=getFormatPrice($aryOrderSaleList[$i][O_STATUS_PRICE4],2,$S_ST_CUR)?></td>

				<td><a href="<?=$strUrl?>&mode=deliveryList&searchOrderStatus=D" target="_blank"><?=NUMBER_FORMAT($aryOrderSaleList[$i][O_STATUS_CNT5])?></a></td>
				<td><?=getFormatPrice($aryOrderSaleList[$i][O_STATUS_PRICE5],2,$S_ST_CUR)?></td>

				<td><a href="<?=$strUrl?>&mode=list&searchOrderStatus=E" target="_blank"><?=NUMBER_FORMAT($aryOrderSaleList[$i][O_STATUS_CNT6])?></a></td>
				<td><?=getFormatPrice($aryOrderSaleList[$i][O_STATUS_PRICE6],2,$S_ST_CUR)?></td>


				<td><a href="<?=$strUrl?>&mode=list&searchOrderStatus=C" target="_blank"><?=NUMBER_FORMAT($aryOrderSaleList[$i][O_STATUS_CNT7])?></a></td>
	...
	<div class="noticeInfoWrap">
		<ul>
			...
			<li class="topLine">- <?= $LNG_TRANS_CHAR["EM00003"]; //<span class="tit">총매출액</span>: "결제완료 + 배송준비중 + 배송중 + 배송완료 + 구매완료"의 합산금액입니다. ?></li>
