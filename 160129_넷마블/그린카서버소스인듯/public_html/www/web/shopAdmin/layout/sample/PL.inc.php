<?
	...
	$strSliderUseCss	= ($skinRow['PL_SLIDER_USE']!="Y") ? "display:none" : "" ;

	for($i=1;$i<=5;$i++) :
		$skinRow['PL_BEST_LIST'.$i.'_DESIGN'] = ($skinRow['PL_BEST_LIST'.$i.'_DESIGN']) ? $skinRow['PL_BEST_LIST'.$i.'_DESIGN'] : "PB0001"; 
	endfor;
	$skinRow['PL_BEST_LIST1000_DESIGN'] = ($skinRow['PL_BEST_LIST1000_DESIGN']) ? $skinRow['PL_BEST_LIST1000_DESIGN'] : "PL0001";
?>
...
//	function goSubBestImg(ic_code)								{selfSubBestImgPopup(ic_code);}							//추천상품 이미지 클릭
	...
	function onPageLoadEvent() {
		...
		imageOnOff($("input[name^=pl_navi_use_op]"));
	}
...
//		C_getAction("skin_pl_InfoSave", "<?=$PHP_SELF?>");
//		return;	

		var doc = document.form;
		doc.mode.value		= "act";
		doc.act.value		= "skin_pl_InfoSave";
...
<div class="detailLayoutSample">
	<ul>
		...
		<? for($i=1;$i<=5;$i++): 
				$strCss = ($skinRow['PL_BEST_LIST'.$i.'_NAME'] == "" || $skinRow['PL_BEST_LIST'.$i.'_USE']!="Y") ? "display:none" : "";				?>
		<li id="pl_best_list<?=$i?>_use" style="<?=$strCss?>">
			<a href="javascript:goMainBestImg('<?=$i?>', '<?=$skinRow['PL_BEST_LIST'.$i.'_DESIGN']?>')">
				<img src="http://www.eumshop.com/upload/shopDesign/subDesign/sub_prodlist_<?=$skinRow['PL_BEST_LIST'.$i.'_DESIGN']?>.jpg" id="imgProdListDesign"/>
			</a>
		</li>
		<? endfor; ?>
		<li id="pl_best_list1000_use"><a href="javascript:goMainBestImg('1000', '<?=$skinRow['PL_BEST_LIST1000_DESIGN']?>')" id="btnSkinProdListDesign">
			<img src="http://www.eumshop.com/upload/shopDesign/subDesign/sub_prodlist_<?=$skinRow['PL_BEST_LIST1000_DESIGN']?>.jpg" id="imgProdListDesign" style="width:412px"/>
		</a></li>
	</ul>
</div>
<div class="layoutSettingInfo">
	...
	<ul>
		<li><input type="checkbox" name="pl_top_use_op_y" id="btnTopImageUse" value="Y"<?if(in_array($skinRow['PL_TOP_USE_OP'], array("A","B"))) { echo " checked"; }?>> 탑이미지 사용</li>
			<div class="subChk">
				<!--input type="radio" name="pl_top_use_op" value="A"<?=($skinRow['PL_TOP_USE_OP']=="A") ? " checked" : "" ?>> 모든 상품페이지에 한개의 이미지만 적용<br--> 
				<!--<input type="radio" name="pl_top_use_op" value="B"<?=($skinRow['PL_TOP_USE_OP']=="B") ? " checked" : "" ?>> 카테고리별 이미지 업로드 적용-->
				<input type="hidden" name="pl_top_use_op">
			</div>
		<li><input type="checkbox" name="pl_sub_cate_use" id="btnSubCategoryUse" value = "Y"<?=($skinRow['PL_SUB_CATE_USE']=="Y") ? " checked" : "" ?>> 서브 카테고리 사용</li>

		<div id="displayUseArea">
			<? for($i=1;$i<=5;$i++): 
					$strCss = ($skinRow['PL_BEST_LIST'.$i.'_NAME'] == "") ? "display:none" : "";				?>
			<li style="<?=$strCss?>">
				<input	type="checkbox" name="pl_best_list<?=$i?>_use" id="pl_best_list<?=$i?>_use" value="Y" "<?=($skinRow['PL_BEST_LIST'.$i.'_USE']=="Y") ? " checked" : "" ?>/><?=$skinRow['PL_BEST_LIST'.$i.'_NAME']?>
			</li>
			<? endfor; ?>
		</div>
