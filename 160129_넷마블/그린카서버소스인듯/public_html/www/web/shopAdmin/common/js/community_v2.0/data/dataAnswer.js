...
//		goEumEditor2Ready(objConfig1, objConfig2);
	...
	$(function() {
		...
		// 포커스 설정
		objTarget.find('[name=ub_title]').focus();
	...
	// 취소
	function goCommunityAnswerViewMoveEvent() {
	...
	// 답변등록2
	function goCommunityAnswerActEvent() {
		...
		// 유효성 체크
		if(!goCommunityDataAnswerFormCheck()) { return; }
	...
	// 유효성 체크
	function goCommunityDataAnswerFormCheck() {
		
		// 기본설정
		var objTarget = $('[name=answerForm]');
		var isStop = false;
	...
	// 첨부파일
	function goCommunityDataAnswerAtcFormToggleEvent() {
	...
	// 첨부파일 업로드
	function goCommunityDataAnswerAtcWriteActEvent() {
		...
		objTarget.ajaxForm({
			...
			success		 :  function(data) {
				...
				if(data['__STATE__'] == "SUCCESS") {
					...
					goCommunityDataAnswerAtcFileDrawEvent(data);
					goCommunityDataAnswerAtcFormToggleEvent();
	...
	// 첨부파일 업로드 이후 그리기
	function goCommunityDataAnswerAtcFileDrawEvent(data) {

		// 기본설정
		var objTarget = $('form[name=answerForm]').find('.atcFileList');
	...
	// 첨부파일 삭제
	function goCommunityDataAnswerAtcDeleteActEvent(intNo) {
