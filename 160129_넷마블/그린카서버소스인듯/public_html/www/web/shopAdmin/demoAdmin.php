<?
	...
	## 기본설정
	$strDemoID = "master";
	if($S_MEM_CERITY != 1) { $strDemoID = "demoAdmin@eumshop.co.kr"; } // 로그인 타입이 이메일.
				...
				<div class="loginForm">
						<?php if($S_MALL_TYPE == "M"):?>
							<div class="loginTabBox">
								<a href="./" class="on"><strong>마스터 관리자 로그인</strong></a>
								<a href="./sellerLogin.php"><strong>입점사 로그인</strong></a>
							</div>
						<?php endif;?>
						...
						<ul>
							<li><input type="text" name="id" id="id" value="<?php echo $strDemoID;?>" tabindex="1" onkeydown="if(event.keyCode==13) document.form.pass.focus();" onFocus="this.className='input_focus'" onBlur="if ( this.value == '' ) { this.className='input_id_blur' }" class="input_id_blur"/></li>
							<li><input type="password" name="pass" id="pass" value="111111" tabindex="2" onkeydown="if(event.keyCode==13) goLogin();"  onFocus="this.className='input_focus'" onBlur="if ( this.value == '' ) { this.className='input_pw_blur' }" class="input_pw_blur"/></li>
							<li><a href="javascript:goLogin();" class="btns"><img src="/shopAdmin/himg/login/btn_login.png" tabindex="3"/></a></li>
							<li><span class="on_off"><input type="checkbox" name="chkAutoLogin" tabindex="4" id="on_off" value="Y" <?=($strAutoLoginId)?"checked":"";?>/></span> <label class="on_txt">아이디저장</label></li>
						</ul>
						</form>
				</div><!-- loginForm -->
			</div><!-- loginWrap-->
