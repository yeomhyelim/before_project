<?
	...
	case "prodOrderUpdate":
		// 상품우선순위수정
			...
			## 기본 설정
			$strProductOrderList			= $_POST['pCodeList'];
			$intAdminNo						= $_SESSION['ADMIN_NO'];
			
			## 기본 설정 체크
			if(!$strProductOrderList || !is_array($strProductOrderList)):
			...
			## 상품우선순위 수정
			foreach($strProductOrderList as $strPCode => $intProductOrder):
				
				$param						= "";
				$param['P_CODE']			= $strPCode;
				$param['P_ORDER']			= $intProductOrder;
				$param['P_MOD_DT']			= "NOW()";
				$param['P_MOD_NO']			= $intAdminNo;
