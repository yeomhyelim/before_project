<?
	...
	## 검색 카테고리
	//$strStLngLower 사용시 언어 탭별로 바뀜.
	//include_once SHOP_HOME."/conf/category.{$strStLngLower}.inc.php";

	include_once SHOP_HOME."/conf/category.".strtolower($S_SITE_LNG).".inc.php";
	...
	<tr>
		...
		<td>
			<?foreach($aryProdMainDisplayList as $key => $data):
				...
				<input type="checkbox" id="searchMainDisplay" name="searchMainDisplay[]" value="<?=$data['IC_CODE']?>"<?if(in_array($data['IC_CODE'], $arrSearchProdMainDisplay)){echo "checked";}?>><?= $data['IC_NAME'] == "메인추천상품" ? $LNG_TRANS_CHAR["PW00292"] : $data['IC_NAME'] ?>
			<?endforeach;?>
		</td>
	</tr>
	<?endif;?>
