<?
	...
	$aryScriptEx[] = "./common/js/product/product/prodWrite.js";
?>
...
//<![CDATA[
    ...
    //이미지 업로드 사이즈 체크. 남덕희 2015.07.07.
	$(document).ready(function(){
	   $("input[name^=prodImg]").live("change", function () {
            ...
            if( 5242880 < fileSize)
            {
                ...
                alert("5Mbyte 이상 파일은 업로드 할  수 없습니다.");
            }
        });

	    //상품 옵션 속성 focus시 문구 사라지게 함. 남덕희 2015.8.4
		$( "input[id^=prodOptVal]" ).focus(function() {
			$(this).val('');
    	}).blur(function() {
      		//if($(this).val() == "") { $(this).val("Ex. 10kg;20kg(옵션별 속성 구분은 세미콜론';'을 입력해주세요.)"); }
    	});
	});
//]]>
</script>
...
th, td {letter-spacing:0 !important;}
.helpTxt {letter-spacing:0 !important;}
</style>

<input type="hidden" name="prodShopCountry" value="<?=$strStLng?>">
...
<div id="contentArea">
	...
	<div class="tableFormWrap">
		...
		<table class="tableForm">
			<tr>
				...
				<td>
					...
					<input type="radio" id="" name="prodWebViewYN" value="Y" checked/><?=$LNG_TRANS_CHAR["PW00119"] //보임?>
					<input type="radio" id="" name="prodWebViewYN" value="N" /><?=$LNG_TRANS_CHAR["PW00120"] //안보임?>
					 , <?= $LNG_TRANS_CHAR["PW00012"]; //모바일 ?>:
					<input type="radio" id="" name="prodMobViewYN" value="Y" checked/><?=$LNG_TRANS_CHAR["PW00119"] //보임?>
					<input type="radio" id="" name="prodMobViewYN" value="N" /><?=$LNG_TRANS_CHAR["PW00120"] //안보임?>
					<?}else{?>
				...
				<td>
					<input type="text" <?=$nBox?>  style="width:150px;" id="prodRepDt" name="prodRepDt" value="<?=($prodRow[P_REP_DT]) ? SUBSTR($prodRow[P_REP_DT],0,10) : date("Y-m-d"); ?>" readonly />
				</td>
			</tr>
			...
			<?if($a_admin_type == "A"){?>
				...
				<tr>
					...
					<td>
						<input type="text" <?=$nBox?>  style="width:150px;" id="prodOrder" name="prodOrder" value="<?=($prodRow[P_ORDER] == 0) ? '9999' : $prodRow[P_ORDER];?>" maxlength="4"/>
					</td>
				...
				<?}?>
			<?}else{?>
				<input type="hidden" name="prodOrder" value="9999" maxlength="4"/>
			<?} ?>
		...
		<table class="tableForm">
			...
			<tr>
				...
				<th><span class="mustItem">Type</span></th>
				<td >
			...
			<tr>
				...
				************/
				...
				<td>
					...
					<!--
					<input type="text" <?=$nBox?>  style="width:150px;" id="prodOrigin" name="prodOrigin" readonly/>
			...
			<tr>
				...
				<td colspan="3">
					<input type="radio" id="priceFilter" name="priceFilter" value="EXW" <?=(($prodRow[P_PRICE_FILTER]=="EXW" || $prodRow[P_PRICE_FILTER]=="") && $strStLng == 'KR')?" checked ":"";?> onclick="javascript:goPriceFilter(this);" <?=($strStLng != 'KR')?" disabled ":"";?> />EXW
					<input type="radio" id="priceFilter" name="priceFilter" value="FOB" <?=($prodRow[P_PRICE_FILTER]=="FOB" || $strStLng != 'KR')?" checked ":"";?> onclick="javascript:goPriceFilter(this);"/>FOB
					...
					<span id='priceUnit'>
					<?
					if($prodRow[P_PRICE_FILTER] == 'FOB' || $strStLng != 'KR')	{
						echo "$";
					}else{
			...
			<tr>
				<th>CAS No</th>
				<td>
					<input type="text" <?=$nBox?>  style="width:100px;text-align:right;font-weight:bold;" id="prodCASNo" name="prodCASNo" value="<?=$prodRow[P_CAS_NO]?>" />
				</td>
				<th><span class="mustItem"><?=$LNG_TRANS_CHAR["PW00040"] //수량?></span></th>
				<td>
					<input type="text" <?=$nBox?>  style="width:100px;text-align:right;font-weight:bold;" id="prodQty" name="prodQty" value="<?=$prodRow[P_QTY]?>" <?//=($prodRow[P_QTY]==0)?"disabled":"";?>  maxlength = "20" />
					...
					<input type="checkbox" id="prodStockOut" name="prodStockOut" value="Y" <?=($prodRow[P_STOCK_OUT]=="Y")?"checked":"";?> maxlength = "255" /> <?=$LNG_TRANS_CHAR["PW00041"] //품절?>
			...
			<tr>
				...
				<td>
					...
					<a href="./?menuType=product&mode=prodBrandList" class="btn_sml"><span><?=$LNG_TRANS_CHAR["PW00222"]//브랜드관리?></span></a>
				</td>
			...
			<tr>
				...
				<td>
					<?	## 다국어출력사용여부
						if ($S_PROD_MANY_LANG_VIEW == "Y"){
					?>
					<input type="radio" id="" name="prodWebViewYN" value="Y" checked/><?=$LNG_TRANS_CHAR["PW00119"] //WEB 보임?>
					<input type="radio" id="" name="prodWebViewYN" value="N" /><?=$LNG_TRANS_CHAR["PW00120"] //모바일 보임?>
					<?}else{?>
					<input type="checkbox" id="" name="prodWebViewYN" value="Y" checked/><?=$LNG_TRANS_CHAR["PW00011"] //WEB 보임?>
					<input type="checkbox" id="" name="prodMobViewYN" value="Y" checked/><?=$LNG_TRANS_CHAR["PW00012"] //모바일 보임?>
					<?}?>
		...
		<table class="tableForm">
			<tr>
				...
				<td>
					...
					<?=$S_ARY_MONEY_ICON[$strProdLng]["R"]?>
					(<?=$LNG_TRANS_CHAR["MW00031"]//할인율?>:<input type="text" <?=$nBox?>  style="width:40px;text-align:right;font-weight:bold;" id="prodDiscountRate" name="prodDiscountRate" maxlength="3"/>%)
					<span class="helpTxt">
	...
	<div id="prodOptionArea" style="display:">
		<div class="bBoxWrap">
			...
			<input type="hidden" id="prodOptType" name="prodOptType" value="3" checked/><!-- <?=$LNG_TRANS_CHAR["PW00053"] //다중가격분리형?> -->
		</div>
		<div class="tableInList">
			<table id="tabProdOpt" class="tableForm">
				...
				<tr>
					...
					<th><span class="mustItem"><?=$LNG_TRANS_CHAR["PW00055"] //옵션속성?></span></th>
					<th><span class="mustItem"><?=$LNG_TRANS_CHAR["PW00056"] //필수사항?></span></th>
				</tr>
				<tr>
					<td><input type="text" <?=$nBox?>  style="width:120px;" id="prodOptName1" name="prodOptName1" value="Packing" /></td>
					<td style="text-align:left;"><input type="text" <?=$nBox?>  style="width:99%;" id="prodOptVal1" name="prodOptVal1" value="<?=$LNG_TRANS_CHAR["PS00082"] //Ex. 10kg;20kg(옵션별 속성 구분은 세미콜론';'을 입력해주세요.?>"/></td>
					<td><input type="checkbox" id="prodOptEss1" name="prodOptEss1" value="Y" checked readonly /></td>
				</tr>
			</table>
			...
			<table id="tabProdOptAttr" class="tableForm"></table>
			<div class="bBoxWrap">
					<div class="helpTxt">
						<!--
					...
					<div class="hefpTxtBox">
						<!--
						<strong><?= $LNG_TRANS_CHAR["PS00073"]; //옵션속성 만들기 방법 ?></strong><br/>
						① <?= $LNG_TRANS_CHAR["PS00074"]; //옵션명과 옵션항목을 입력합니다. 옵션항목은 세미콜론(;)으로 구분합니다. 예) Black;Red;Yellow ?><br/>
						② <?= $LNG_TRANS_CHAR["PS00075"]; //선택할 옵션값이 더 있다면 추가 버튼을 클릭하여 한줄을 더 생성합니다. 예) Size: XL;L;S ?><br/>
						③ <?= $LNG_TRANS_CHAR["PS00076"]; // 생성할 옵션항목이 입력되었다면 "옵션속성만들기" 버튼을 클릭하세요.  하단에 각 항목이 분리되어 정렬됩니다. ?><br/>
	...
	<div class="tableFormWrap">
		...
		<div id="prodAddArea" style="display:none">
				...
				<tr class="prodItem1">
					...
					<td>
						<select name="prodItemType[]" id="prodItemType[]" onchange="javascript:goProdItemTypeSelect(this);">
							<option value="B"><?= $LNG_TRANS_CHAR["PW00206"]; //설명 ?></option>
							<!--<option value="U"><?= $LNG_TRANS_CHAR["PW00285"]; //사용자입력 ?></option>-->
						</select>
			...
			<?}?>
			<?}?>
			<div class="helpTxt bBoxWrap">
				* <?=$LNG_TRANS_CHAR["PS00002"] //상품 요약 정보에 추가적으로 문구를 넣으실 수 있습니다.?>
			</div>
		</div>
	</div>
	<?php include $strIncludePath.$aryIncludeFolder[$strMode]."/prodNotify.info.inc.php";?>
	...
	<div class="tableFormWrap">
		...
		<table id="tabProdImg2" class="tableForm">
			<? ## 2014.04.05 상세이미지 다중이미지로 변경 ?>
			<? ## 2014.04.11 한장의 이미지로 사용하기 - 추가 ?>
			<tr id="prodImgTrId_4">
				<th><span class="mustItem"><?=$LNG_TRANS_CHAR["PW00074"] //확대이미지?></span></th>
				<td colspan="3">
					...
					<!-- <input type="checkbox" id="prodImgCopy" name="prodImgCopy" value="Y"  <?=$nBox?>  style="height:20px;" checked /> <?=$LNG_TRANS_CHAR["PS00049"] //한장의 이미지로 사용하기?> -->
					<!-- <input type="checkbox" id="prodImgUrlYN" name="prodImgUrlYN" value="Y"  <?=$nBox?>  style="height:20px;" /> <?=$LNG_TRANS_CHAR["PS00050"] //이미지 URL 사용하기?> -->
					<div class="helpTxt">
			...
			<tr id="productDetailImage_1">
				...
				<td colspan="3">
					...
					<input type="file" id="prodImgBasic" name="prodImg7" value="" <?=$nBox?> style="height:20px;"/>
					<!-- <?=$LNG_TRANS_CHAR["PW00225"] //확대?> : <input type="file" id="prodImgExpend" name="prodImg18" value="" <?=$nBox?> style="height:20px;"/> -->
				</td>
			</tr>
		</table>
		<table id="tabProdImg3" style="display:none" class="tableForm">
			...
			<tr id="productDetailUrlImage_1">
				...
				<td colspan="3">
					<?=$LNG_TRANS_CHAR["PW00224"] //기본?> : <span><input type="text" id="prodUrlImgBasic" name="prodUrlImg7" value="" <?=$nBox?> style="width:350px;"/></span>
					<?=$LNG_TRANS_CHAR["PW00225"] //확대?> : <span><input type="text" id="prodUrlImgExpend" name="prodUrlImg18" value="" <?=$nBox?> style="width:350px;"/></span>
				</td>
			</tr>
		</table>
		...
		<table class="tableForm">
			<?if($siteCommArray):?>
			<tr>
				<th><?= $LNG_TRANS_CHAR["PW00286"]; //공통 리스트 ?></th>
			...
			<tr>
				...
				<td>
					...
					<textarea name="prodWebText" style="display:none"></textarea>
					<div class="helpTxt">
			...
			<table class="tableForm">
				...
				<tr>
					...
					<td colspan="3">
						...
						<textarea name="prodMobileText" style="display:none"></textarea>
						<div class="helpTxt">
...
<div class="buttonWrap" style="margin:0px;padding-top:5px;padding-bottom:5px;border-top:5px solid #5e5e6d;position:Fixed;left:175px;bottom:0px;width:100%;background:#fff;">
	<a class="btn_blue_big" href="javascript:void(0);" onclick="goProductProdWriteActEvent();" id="menu_auth_w" style="display:none"><strong><?=$LNG_TRANS_CHAR["CW00002"] //등록?></strong></a>
