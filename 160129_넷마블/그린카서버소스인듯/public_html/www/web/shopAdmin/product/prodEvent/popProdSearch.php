...
<?
	...
	if (($strSearchField && $strSearchKey) || $strC_HCODE1) {
		...
		$linkPage .= "&searchField=$strSearchField&searchKey=$strSearchKey&no=$intSE_NO&page=";
	}
...
<!--
	...
	function goProdCateAllInsert()
	{
		...
		C_getAjax("prodEventCateAllReg","act");
	}
	...
	function goProdAllInsert()
	{
		...
		C_getAjax("prodEventProductReg","act");
	}

	function goAjaxRet(name,result){

		if (name == "prodEventCateAllReg" || name == "prodEventProductReg")
		{			
			...
			if (data[0].RET == "N")
			{
				alert("<?=$LNG_TRANS_CHAR['PS00034']?>"); //카테고리별로 이벤트 상품 등록중 오류가 발생하였습니다.
				return;
			}

			if (data[0].RET == "Y")
			{
				alert("<?=$LNG_TRANS_CHAR['PS00033']?>");	//카테고리별로 이벤트 상품이 등록되었습니다.		
				parent.location.reload();
				return;
			}
		}
		...
		<table>
			...
			<tr>
				<th>
					...
					<a class="btn_sml" href="javascript:goSearch('popProdSearch');"><strong><?=$LNG_TRANS_CHAR["CW00027"] //검색?></strong></a>
				</th>
			</tr>
			<tr>
				<th>
