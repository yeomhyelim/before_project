	<div class="tableList">
		<table style="border-left:1px solid #D2D0D0">
			...
			<?}else{
				while($row = mysql_fetch_array($orderListResult)){
					...
					if (in_array($row[O_STATUS],array("B","I","D","E","C","S","R","T"))){
						$strOrderStatusText = $S_ARY_SHOP_ORDER_STATUS[$row[O_STATUS]];
						if (!$strOrderStatusText) $strOrderStatusText = $S_ARY_DELIVERY_STATUS[$row[O_STATUS]];
					}
			...
			<tr>
				...
				<td>
					<?=$btnOrderSettleStatusUpdate?>
					<? if ($row['O_APPR_DT']){echo "<br>[".$row['O_APPR_DT']."]";}?>
				</td>
							...
							<?
								...
								<tr>
									...
									<td alt="상품이미지" style="text-align:left;">
										<?=$intCartRowSpan?><img src="<?=$aryOrderCartList[$i]['PM_REAL_NAME']?>" style="width:50px;height:50px">
									</td>
