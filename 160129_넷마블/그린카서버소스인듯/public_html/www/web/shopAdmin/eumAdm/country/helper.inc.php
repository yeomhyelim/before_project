<?
	...
	switch($strMode){
		case "countryList":
			...
			if (!$strSearchDeliveryMth) $strSearchDeliveryMth = "E";
			...
			$intTotal	= $eumMgr->getCountryList($db,"countryCount",$param);
			...
			$result = $eumMgr->getCountryList($db,"countryList",$param);
			//echo $db->query;
			...
			$linkPage .= "&searchDeliveryMth=$strSearchDeliveryMth";
			$linkPage .= "&searchCountryZone=$strSearchCountryZone";
			$linkPage .= "&page=";

			$aryCountrySelectList = $eumMgr->getCountrySelectList($db);
