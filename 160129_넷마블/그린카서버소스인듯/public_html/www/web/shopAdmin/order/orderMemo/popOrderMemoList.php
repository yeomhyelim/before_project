			...
			<table>
				...
				<tbody>
					<?while($row = mysql_fetch_array($boardDataResult)):
						...
						$strProdImg			= $row['PM_REAL_NAME'];
						$strPCode			= $row['UB_P_CODE'];
					...
					<tr id="reportList_<?=$no?>">
						...
						<td class="no"><a href="javascript:goProductViewPageMoveEvent('<?=$strPCode?>')" target="_blank"><img src="<?=$strProdImg?>" style="width:45px;height:45px"></a></td>
						<td class=""><?=$cateName?></td>
						<td class="" style="text-align:left"><a href="javascript:goOrderMemoModifyMoveEvent('<?=$no?>');"><?=$text?></a></td>
