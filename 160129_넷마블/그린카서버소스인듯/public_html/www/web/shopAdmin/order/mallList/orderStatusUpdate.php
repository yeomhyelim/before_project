<?
	switch($strOrderStatus){
		case "A":
			...
			/* SHOP 배송정보를 '배송준비중'으로 변경 */
			$param_shop_update							= "";
			$param_shop_update['o_no']					= $orderRow['O_NO'];
			$param_shop_update['o_status']				= "";
			$param_shop_update['so_delivery_status']	= "B";
			$shopOrderMgr->getOrderAccStatusUpdate($db,$param_shop_update);
...
//				endif;
		...
		case "I":
			...
			if ($orderRow['O_USE_LNG'] != "KR"){
				...
				$aryTAG_LIST['{{__주문상품내역__}}']	= $strOrderCartHtml;
				$aryTAG_LIST['{{__주문금액정보__}}']	= $strOrderCartPriceHtml;
				$aryTAG_LIST['{{__주문배송내역__}}']	= $strOrderInfoHtml;
				goSendMail("013");
				/** 메일 전송 **/
			}
		break;
...
//						$param['so_delivery_com']		= $aryDeliveryCom[$orderDeliveryRow['SO_DELIVERY_COM']];
//						$param['so_delivery_num']		= $_POST["deliveryNum_{$so_no}"];
						
						$tno		= $orderRow[O_APPR_NO];
						$req_tx		= "mod_escrow";
						$mod_type	= "STE1";
						$deli_corp	= $aryDeliveryCom[$param['so_delivery_com']];
						$deli_numb	= $param['so_delivery_num'];
//						echo $deli_corp.":".$deli_numb."<br>"; 
...
//									echo $intCnt.":".$strOrderKey.":".$intO_NO.":".$orderRow['O_STATUS'].":실패<br>";
//							}
