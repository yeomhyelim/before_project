				...
				<td>
					...
					if($strBI_USERFIELD_USE == "Y"){
						...
						foreach($aryUserfieldList as $key => $data){
							...
							if( $strUSE == 'Y' )
							{
								$aryTempData = explode(';',$data["KIND_DATA"]);
								// 
								switch($strKIND)
								{
									case("select"):
										...
										$aryUserfieldView .= ">";
										for($j=0;$j < sizeof($aryTempData);$j++)
										{
											$aryUserfieldView .= "<option value='".$aryTempData[$j]."'";
											if($aryTempData[$j] == $data['KIND_DEFAULT'])
											{
												$aryUserfieldView .=	" selected ";
											}
											$aryUserfieldView .= ">";
											$aryUserfieldView .=	$aryTempData[$j];
