							...
							for($j=0;$j<sizeof($aryProdAddAttrOpt);$j++){
								...
								if ($aryProdAddAttrOpt[$j][PBA_NO] > 0)
								{
									...
									if ($S_ARY_CUR[$S_SITE_LNG]["USD"][2] == "Y") {
										$intOrderTotalPriceCur	= $intOrderTotalPriceCur + (getCurToPriceSave($aryProdAddAttrOpt[0][PAO_PRICE],"US") * $aryProdAddAttrOpt[$j][PBA_OPT_QTY]); //현지통화
									} else {
										$intOrderTotalPriceCur	= $intOrderTotalPriceCur + (getCurToPriceSave($aryProdAddAttrOpt[0][PAO_PRICE]) * $aryProdAddAttrOpt[$j][PBA_OPT_QTY]); //현지통화
									}
				...
				if (is_array($aryCouponUseIssueNo)){
					...
					for($i=0;$i<sizeof($aryCouponUseIssueNo);$i++){
						...
						if ($aryCouponInfoList[0][CU_PRICE_OFF] == "1") {
								$intCouponPrice		= getRoundUp($intOrderTotalPrice * ($aryCouponInfoList[0][CU_PRICE]/100),2);
...
//			$orderMgr->setO_TAX_PRICE(getCurToPriceSave($intOrderTaxTotal)); //기준통화->현지통화
			...
			if ($strO_SETTLE == "B" || $strO_SETTLE == "P"){
				...
				if($S_SITE_LNG == "KR" && $SMS_INFO['S_SMS_USE']=="Y"):
					...
					## 코드 설정
					$strSmsCode = "008"; // 무통장입금일때 SMS코드
					if($strO_SETTLE == "P" && $orderRow['O_TOT_CUR_SPRICE'] <= 0) { $smsCode = "012"; } // 포인트 결제시 SMS코드
				...
				for($i=0;$i<sizeof($aryCartNo);$i++){
					...
					if ($aryCartNo[$i] > 0){
						...
						if ($cartRow[PB_OPT_NO]){
							...
							$intOrderTotalPriceCur	= $intOrderTotalPriceCur	+ $intOrderPriceCur; //현재통화
							$intOrderTotalPriceOrg	= $intOrderTotalPriceOrg	+ getCurToPriceSave($intOrderPrice);
