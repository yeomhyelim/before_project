...
//			alert("<?=$LNG_TRANS_CHAR['OS00014']?>"); //로그인을 하신 후 이용하세요.
	 ...
	 */
		...
		strJsonParam += "&prodCode="+prodCode;
						...
						$.getJSON("./?menuType=product&mode=json&act=prodOptAttrNo"+strJsonParam,function(data){
							...
							var intProdOptPrice = "<?
								if ($prodRow[P_EVENT] > 0 && getProdEventInfo($prodRow) == 'Y')
								{
									...
									//echo getCurToPrice($prodRow[P_SALE_PRICE],'US');
									echo getCurToPriceFilter($prodRow['P_SALE_PRICE'],'','',$prodRow['P_PRICE_FILTER']);
								}?>";
							...
							intProdOptPrice2 = "<?
									if ($prodRow[P_EVENT] > 0 && getProdEventInfo($prodRow) == 'Y'){
										...
										//echo getCurToPrice($prodRow[P_SALE_PRICE]);
										echo getCurToPriceFilter($prodRow['P_SALE_PRICE'],'','',$prodRow['P_PRICE_FILTER']);
									}?>";
							<?
							}else{
							...
							var intProdOptPrice = "<?
								if ($prodRow[P_EVENT] > 0 && getProdEventInfo($prodRow) == 'Y')
								{
									...
									{
										//echo getCurToPrice($prodRow[P_SALE_PRICE],'US');
										echo getCurToPriceFilter($prodRow['P_SALE_PRICE'],'US','',$prodRow['P_PRICE_FILTER']);
									}else{
										//echo getCurToPrice($prodRow[P_SALE_PRICE]);
										echo getCurToPriceFilter($prodRow['P_SALE_PRICE'],'','',$prodRow['P_PRICE_FILTER']);
									}
	...
	function goSelectProdOptHtml(optNo,optVal,optPrice,optPrice2)
	{
		...
		strSelectProdOptHtml += "    <td class=\"optPrice\"><?=getCurMark()?>  <?=$strTextPriceUnit?> <strong id=\""+optNo+"_cartOptPriceMark\">"+optPrice+"</strong>";

		//strSelectProdOptHtml += "<?=$strTextPriceUnit?>";
		strSelectProdOptHtml += "</td>";
		<?}?>
	...
	function goSelectProdOptTotalHtml()
	{
		...
		if (intCartOptPriceTotal > 0)
		{
			...
			<?}else{?>
			strSelectProdOptTotalPriceHtml += "<?=$LNG_TRANS_CHAR['PW00042'];?>:<strong class=\"totalPriceTxt\"><?=getCurMark()?> <?=$strTextPriceUnit?></strong><strong id=\"cartOptTotalPrice\" class=\"totalPrice\">"+intCartOptPriceTotal+"</strong><strong class=\"totalPriceTxt\">";

			//strSelectProdOptTotalPriceHtml += "<?=$strTextPriceUnit?>";
			strSelectProdOptTotalPriceHtml += "</strong>";
			<?}?>
