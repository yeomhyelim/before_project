...
if (!$g_member_no)
{
	...
	{
		//echo "<script>parent.location.href='./?menuType=member&mode=login&returnMenu=product&returnMode=view&returnParam=prodCode^||^$strP_CODE';</script>";
		goUrl("","./?menuType=member&mode=login&layout=product&returnMenu=product&returnMode=prodInquiry&returnParam=searchOrderStatus^||^$strSearchOrderStatus");
	}
	exit;
}
...
$intP_SHOP_NO  = $prodRow[P_SHOP_NO];
...
<script type="text/javascript">
	...
	//제품문의
	function goProdInquiry(strAppID) {
...
<div id="<?php echo $strAppID?>" class="popProdInquiryBox">
	<div class="titWrap">
		<h2><?= $LNG_TRANS_CHAR["PW00102"]; //제품문의 ?></h2>
	...
	<div class="prodInquiryWrap">
		...
		<input type="hidden" name="ub_name" value="<?php echo $strUbName;?>">
		<input type="hidden" name="ub_shop_no" value="<?php echo $intP_SHOP_NO;?>">
		<?
		if($strDevice != 'mobile')
		{
		?>
		...
		<table class="prodInquiryTable">
			<tr class="inquiryRow">
				<th><?= $LNG_TRANS_CHAR["PW00103"]; //제품 ?></th>
				<td>
					<ul>
						<li class="prodImg"><img src="<?php echo $strPM_REAL_NAME;?>" class="listProdImg"/></li>
						<li class="prodInfo">
							<p class="title"><?=$strP_NAME?></p>
							<p class="info"><span><?= $LNG_TRANS_CHAR["PW00028"]; //원산지 ?></span> <strong><?php echo $aryCountryList[$strP_ORIGIN]?></strong></p>
							<p class="info"><span><?= $LNG_TRANS_CHAR["CW00064"]; //카테고리 ?></span> <strong><?php echo $aryCateNames1[$strP_CATE]?></strong></p>
						</li>
					</ul>
				</td>
			</tr>
			<tr>
				...
				<td><input type="text" name="ub_title" check="empty" alt="<?php echo $LNG_TRANS_CHAR['CW00062'];//제목?>" /></td>
			</tr>
			<tr>
				<th><?php echo $LNG_TRANS_CHAR["CW00063"];//내용?></th>
				<td><textarea name="ub_text" id="ub_text" title="<?php echo $strBI_DATAWRITE_FORM;?>" style="width:98%;height:200px;" check="empty" alt="<?php echo $LNG_TRANS_CHAR["CW00063"]; // 내용?>"></textarea>
				<?php /* if($S_COMMUNITY_EDTOR == "eumEditor2"):?>
						<?php
						
							include MALL_SHOP . "/common/eumEditor2/editor1.php";?>
					...
					if($strBI_USERFIELD_USE == "Y"){
						...
						foreach($aryUserfieldList as $key => $data){
							...
							if( $strUSE == 'Y' )
							{
								...
								{
									case("select"):
										$aryUserfieldView .= "<select name='{$strKeyLower}'  style=\"width:110px;\"";
										if($strESSENTIAL == "Y"){
	...
	<div class="btnCenter">
		<a href="javascript:goPopClose();" class="btnCancel"><?= $LNG_TRANS_CHAR["MW00044"]; //취소 ?></a>
		<a href="javascript:goProdInquiry('<?=$strAppID?>');" class="btnInquiry btn_red"  <?=$strLogBasketBtnEvent?>><span><?= $LNG_TRANS_CHAR["PW00091"]; //문의하기 ?></span></a>
	</div>
</div>
</body>
