...
<div class="subProdList">
	...
	<div class="prodList mt20">
		<table>
			<?if ($intTotal == 0){?>
			<tr>
				<td colspan="4"><?=$LNG_TRANS_CHAR["PS00001"] //등록된 상품이 없습니다.?></td>
			</tr>
			<?}else{
				...
				<td <?if ($k % $intPageDesignLine == 0) echo "style='width:".$S_PRODUCT_LIST_ISW."px;'";?>>
					<div class="prodWrap">
						...
						<ul style="width:<?=$S_PRODUCT_LIST_ISW?>px;">
							<li class="title"><a href="javascript:goProdView('<?=$row[P_CODE]?>');"><span><?=$row[P_NAME]?></span></a></li>
							<?if ($row[P_CONSUMER_PRICE]>0){?><li class="priceOrg"><s><span><?=NUMBER_FORMAT($row[P_CONSUMER_PRICE])?>원</span></s></li><?}?>
							<li class="priceBoldGray"><span><?=NUMBER_FORMAT($row[P_SALE_PRICE])?>원</span></li>
