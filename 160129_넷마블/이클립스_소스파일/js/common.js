...
function onResize(grid){
	...
	for(var i = grid.length-1; i>=0; i--){
		...
		if(reParents != null){
			...
			var scrollWarpWidth = $("#gbox_" + grid[i]).parents(".tbl-scroll-wrap").width();
			if(scrollWarpWidth != 0) {
...
/*
 * 화면 리사이즈 (그리드 높이가 자동일때)
 * */
function onResizeHeightAuto(grid){

	for(var i = 0; i<grid.length; i++){
		
		var scrollWarpWidth = $("#gbox_" + grid[i]).parents(".tbl-scroll-wrap").width();
		if(scrollWarpWidth != 0) {
			$("#gbox_" + grid[i]).parents(".tbl-scroll-wrap").css({height: "auto"});
		}
		var topWidth = $("#gbox_" + grid[i]).parents(".tbl-scroll-wrap").width() -2;
		// box
		$("#gbox_" + grid[i]).css({width : topWidth + "px" });
		...
		$("#gview_" + grid[i]).find(".ui-jqgrid-hdiv").css({width : topWidth + "px"});
		// Grid_Body
		$("#gview_" + grid[i]).find(".ui-jqgrid-bdiv").css({width : topWidth + "px", height: "auto"});
